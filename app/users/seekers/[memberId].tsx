...
// --- Status Badge Helper (copy from SeekersCalledScreen) ---
...
export default function SeekersListScreen() {
  ...
  const [allSeekers, setAllSeekers] = useState([]); // original unfiltered data
  const [seekers, setSeekers] = useState([]);        // displayed (filtered) data
  ...
  // --- Fetch all seekers for this member (original logic preserved) ---
  ...
  // --- Client-side filtering (applied on top of the fetched list) ---
  const applyFilters = useCallback(
    (overrides = {}) => {
      ...
      if (active.mobile.trim()) {
        result = result.filter(
          (s) => s.mobile && s.mobile.includes(active.mobile.trim())
        );
      }
      ...
      if (active.type !== "") {
        result = result.filter(
          (s) => String(s.type) === String(active.type)
        );
      }
      ...
      if (active.from_date) {
        ...
        result = result.filter(
          (s) => s.created_at && new Date(s.created_at) >= from
        );
      }
      ...
      if (active.to_date) {
        ...
        result = result.filter(
          (s) => s.created_at && new Date(s.created_at) <= to
        );
      }
  ...
  const handleReset = () => {
    ...
    setSeekers(allSeekers); // restore full list
    setFilterVisible(false);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Filter Modal — new compact dot-based design, with Name & Mobile search included */}
...
const styles = StyleSheet.create({
  ...
  // Item Card (original preserved)
  itemCard: {
    backgroundColor: ITEM_BACKGROUND, padding: 15, borderRadius: 12,
  ...
  dateBadge: {
    flexDirection: "row", alignItems: "center",
    backgroundColor: "#F2F2F7", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 4,
  },
  ...
  // ── Filter modal dot rows (prefixed fDot* — no collision with card styles) ──
  ...
  // ── Inline row (label + toggle) for Type and Attended Centre ──
